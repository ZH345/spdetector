...
for (i in 1:length(rho)) {
  ...
  for (j in 1:Nsim) {
    ...
    #fit.y <- predict(sar, newdata = NULL, listw = lisw_nc, pred.type = "TC")
    #ff1 <- predict(sar)
...
## Fit a nonlinear least square model for lower limit
diff.model <- nls(diff25 ~ b * rho^c, data = plot.data,
                  start=list(b = -0.5, c = 2))

summary(diff.model)
xx <- predict(diff.model)
cor(xx,plot.data$diff25)^2

plot.data$diff25.fit <- xx



## Fit a nonlinear least square model for rho and diff
diff.model <- nls(diff975 ~ b * rho^c, data = plot.data,
                  start=list(b = -0.5, c = 2))

summary(diff.model)
xx <- predict(diff.model)
cor(xx,plot.data$diff975)^2

plot.data$diff975.fit <- xx
## mapping
