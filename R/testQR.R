...
for (i in 1:length(rho)) {
  ...
  for (j in 1:Nsim) {
    ...
    b.bias = solve(t(x_mat.temp) %*% x_mat.temp) %*% (t(x_mat.temp) %*% (rho.temp * lag.listw(lisw_nc,d.temp$y)))
    ols.q = lm(y ~ x_mat.temp[,-1], data = d.temp)
    m.temp$coefficients
    ols.q$coefficients - b.bias


    b.bias.vector = x_mat.temp %*% b.bias
    e.ols.q = d.temp$y - predict(ols.q)
    cor.q = as.numeric(cor.test(e.ols.q,b.bias.vector)$estimate)
    q.adjust = (var(b.bias.vector)+2*cor.q*sqrt(var(e.ols.q))*sqrt(var(b.bias.vector)))/var(d.temp$y_tilde)
...
adjust.q.mat = adjust.q + Q.mat
diff.res_act = (R2.sar - adjust.q.mat)/R2.sar*100
...
for (i in 1:length(rho)) {
  ...
  for (j in 1:Nsim) {
    ...
    x_mat.temp <- model.matrix( ~ factor(x1)*factor(x2), data = ddd)
